...
As a result, there are now a huge number of libraries ranging from black-box APIs and no-code solutions to statistically accurate and fine-grained representations. Each of which becoming more bloated as different functionalities are appended.
...
I've personally always been motivated by finding interesting and surprising solutions to problems using the sharpest, hardest tool available.
...
Now everything needed has been defined, I'll create my Gaussian process class. I will simplify the training procedure by making it occur on instanciation. The only other thing that is needed is the ability to call it, returning the mean and variance of the posterior distribution at x.
...
First, I'll need a test function. One of my personal favourite ways of doing this is to sample one from a GP prior:
